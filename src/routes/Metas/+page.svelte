<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    if (pointsToday > 0) {
      ...
      // Enviar puntos y racha al servidor
      const formData = new FormData();
      formData.append('totalPoints', totalPoints.toString());
      formData.append('streak', streak.toString());

      try {
        const response = await fetch('?/submitActivities', {
          method: 'POST',
          body: formData,
        });
        ...
        if (!response.ok) {
          ...
          errorMessage = data.message || 'Error al guardar puntos';
        } else {
          errorMessage = '';
        }
      } catch (error) {
